...
class AreaSelectorDialog(xbmcgui.WindowDialog):
    def __init__(self):
        ...
        self.minisjangButton = xbmcgui.ControlButton(int(1280/2 + x_border), int(720 - y_border - image_y), image_x, image_y, "",
                                                     focusTexture=os.path.join(ADDON.getAddonInfo('path'), 'resources', 'button-minisjang-focus.png'),
                                                     noFocusTexture=os.path.join(ADDON.getAddonInfo('path'), 'resources', 'button-minisjang.png')
                                                     )
        ...
        self.addControls([background, title, self.drTvButton, self.ramasjangButton, self.minisjangButton, self.ultraButton])
